...
const CategoryPage = () => {
  ...
  // Filter articles case-insensitively
  ...
  return (
    <Container className="py-12">
      ...
      <SectionTitle
        title={`${t(displayCategory)} ${t("Portal")}`}
        subtitle={`${t("Explore all dynamic reporting, research, and analysis matching")} "${t(displayCategory)}".`}
      ...
      {filteredArticles.length === 0 ? (
        <div className="py-16 text-center glass-premium rounded-3xl border border-slate-200/40 dark:border-slate-800/30">
          ...
          <p className="text-slate-500 dark:text-slate-455 text-xs mt-2 font-semibold">
            {t("There are currently no mock entries populated for the")} "{t(category)}" {t("category")}.
          </p>
          <Link
            to="/"
